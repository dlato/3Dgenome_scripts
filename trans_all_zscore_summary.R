...
print("# chroms ordered highest to lowest MEAN proportion of sig inters (across all cells)")
meanprop <- totInter %>% group_by(chrom) %>% dplyr::summarize(mProp=mean(prop, na.rm = TRUE))
...
totInter[which(totInter$prop == max(totInter$prop)),]
print("# top 10 chrom pair ordered highest to lowest MEAN proportion of sig inters (across all cells)")
meanprop <- totInter %>% group_by(chrpair) %>% dplyr::summarize(mProp=mean(prop, na.rm = TRUE))
meanprop[order(meanprop$mProp, decreasing = TRUE),]


#print("#circos plot of sig interactions")
...
#  summary(nuc1)
#  filename <- paste0("circos_significant_trans_inters_",i,".pdf")
