...
mdat2 <- dat2 
cellcolnames <- colnames(mdat2)
cellcolnames <- cellcolnames[-c(1,2,3,4,5,6,7)]
mdat2$meanZscore <- rowMeans(mdat2[,cellcolnames])
#save df
write.table(mdat2 %>% select(chrA, st1, end1,chrB,st2,end2,meanZscore), file = outfile, sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
