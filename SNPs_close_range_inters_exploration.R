...
lzdat <- zdat %>% gather(cell, zscore, 8:ncol(zdat)) %>% na.omit() %>% filter(cell %in% cells$V1)
head(lzdat)
#p-values
...
head(lpdat) %>% na.omit() %>% filter(cell %in% cells$V1)
print("merge zscore and pvals")
ldat <- merge(lzdat,lpdat) %>% na.omit() %>% filter(cell %in% cells$V1)
head(ldat)
