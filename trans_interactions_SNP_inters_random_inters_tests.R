...
colnames(testSNP_inters) <- c("chrA","st1","end1","chrB","st2","end2","cell","interFreq")
...
colnames(regSNP_inters) <- c("chrA","st1","end1","chrB","st2","end2","cell","interFreq")
#filter by desired cells
...
colnames(nonRegSNP_inters) <- c("chrA","st1","end1","chrB","st2","end2","cell","interFreq")
#filter by desired cells
...
#wide to long
ALL_inters <- ALL_inters %>% gather(key = "cell", value = "interFreq", 8:ncol(ALL_inters))
head(ALL_inters)
...
permdat_posinterFreq <- function(x,y,z) {
  rbins <- sample(x,y, replace = F)
  rinters <- z %>% filter(cell == c) %>% filter(idA %in% rbins | idB %in% rbins) %>% na.omit() %>%
    filter(interFreq > 0)
  mean(rinters$interFreq)
}
permdat_neginterFreq <- function(x,y,z) {
  rbins <- sample(x,y, replace = F)
  rinters <- z %>% filter(cell == c) %>% filter(idA %in% rbins | idB %in% rbins) %>% na.omit() %>%
    filter(interFreq < 0)
  mean(rinters$interFreq)
...
permdat_numInters <- function(x,y,z) {
  ...
  tarD$AllSt <- tarD$st2
  tarD$AllEnd <- tarD$end2
  rinters_bin <- rbind(anchD,tarD) %>% select(cell,AllChr,AllSt, interFreq)
...
for (c in cells_sub$V1){
  ...
  hisdat_posZ <- as.data.frame(replicate(permnum,permdat_posinterFreq(bin_inters_univ, numBins,inters_univ)))
  ...
  p <- (ggplot(hisdat_posZ,aes(x=posZ))
        ...
        + labs(y = "Frequency",
               x = "Mean normalized interaction frequency per 1Mb bin",
               title = c)
        + scale_x_continuous(expand = c(0, 0))
        + scale_y_continuous(expand = c(0, 0))
  )
  pdf(paste0(outfile,"_",c,"_histogram_positive_interFreq"), width = 14, height = 4)
  print(p)
  dev.off()
  # p-value for positive zscore
  pospval <- ((hisdat_posZ %>% filter(posZ >= mean(tSNP_inters_bin_all$mPosZscore, na.rm = T)) %>% nrow())+1) / (permnum+1)
  #add to table
  tr <- c(c,pospval,"interFreq")
  perm_pvals[nrow(perm_pvals) + 1, ] <- tr
#  hisdat_negZ <- as.data.frame(replicate(permnum,permdat_neginterFreq(bin_inters_univ, numBins,inters_univ)))
...
#        + scale_y_continuous(expand = c(0, 0))
#  )
#  pdf(paste0(outfile,"_",c,"_histogram_negative_interFreq"), width = 14, height = 4)
#  print(p)
#  dev.off()
...
#  tr <- c(c,negpval,"negZscore")
  ...
  p <- (ggplot(hisdat_num,aes(x=numInters))
        ...
        + labs(y = "Frequency",
               x = "Mean # of interactions per 1Mb bin",
               title = c)
        + scale_x_continuous(expand = c(0, 0))
        + scale_y_continuous(expand = c(0, 0))
  )
